...
$permisos = $this->login_user->rol_permisos;

#REQUERIMIENTOS
$elog_menu = get_array_value($permisos, "exlog_menu");
$elog_gerencia = get_array_value($permisos, "exlog_gerencia");
$elog_jefatura = get_array_value($permisos, "exlog_jefatura");
$elog_supervisor = get_array_value($permisos, "exlog_supervisor");
$elog_colaborador = get_array_value($permisos, "exlog_colaborador");
$elog_cliente = get_array_value($permisos, "exlog_cliente");
?>

<div class="toolbar" id="kt_toolbar">
	<div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
		<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
			<h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1">Gestion</h1>
			<span class="h-20px border-gray-200 border-start mx-4"></span>
			<ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
				<li class="breadcrumb-item text-muted">
					<a href="<?php echo base_url(); ?>cliente" class="text-muted text-hover-primary">Cliente</a>
				</li>			
				<li class="breadcrumb-item">
					<span class="bullet bg-gray-200 w-5px h-2px"></span>
				</li>
				<li class="breadcrumb-item text-dark">Años</li>
				<li class="breadcrumb-item">
					<span class="bullet bg-gray-200 w-5px h-2px"></span>
				</li>
				<li class="breadcrumb-item text-dark">Meses </li>
				<li class="breadcrumb-item">
					<span class="bullet bg-gray-200 w-5px h-2px"></span>
				</li>
				<li class="breadcrumb-item text-dark">Carpetas </li>
				<li class="breadcrumb-item">
					<span class="bullet bg-gray-200 w-5px h-2px"></span>
				</li>
				<li class="breadcrumb-item text-dark">Archivos </li>
			</ul>
		</div>
		...
		<div class="card mb-5 mb-xl-10" id="kt_profile_details_view">
			...
			<div class="card-header pt-8">
				<div class="card-title">
					...
					<div class="badge badge-lg badge-light-primary">
						<div class="d-flex align-items-center flex-wrap">
							...
							<!--end::Svg Icon-->
							<?php echo $file->EX_CARPETA_NOMBRE; ?>
						</div>
					</div>
					<!--end::Folder path-->
				</div>
				...
				<div class="card-toolbar">
					<?php 
						$mes = $mes->EX_CODIGO_MES + 01;
						$mes_final = sprintf("%02d",$mes);
						$dhabil = $anio->EX_ANIO_HABILITAR_DIA;
						//$hoy = date('Y-m-d');
						$hoy = '2022-04-03';
						$año = date('Y');
						$f_inicio = $año.'-'.$mes_final.'-01';
						$f_fin = $año.'-'.$mes_final.'-'.$dhabil;
						//echo $f_inicio.' '.$f_fin.' '.$hoy;
						if (($this->login_user->IS_DEVELOPER || $elog_gerencia || $elog_jefatura || $elog_supervisor || $elog_colaborador)) {
							if ($hoy >= $f_inicio && $hoy <= $f_fin) { ?>
								<!--begin::Toolbar-->
								<div class="d-flex justify-content-end" data-kt-filemanager-table-toolbar="base">
									<!--begin::Export-->
									<button type="button" class="btn btn-light-primary me-3" data-bs-toggle="modal" data-bs-target="#mAddDoc">
				...
				<table id="kt_file_manager_list" data-kt-filemanager-table="folders" class="table align-middle table-row-dashed fs-6 gy-5">
					...
					<tbody class="fw-bold text-gray-600">		
						<?php foreach ($docs as $doc): 
							if ($doc->ARC_ESTADO == 1) {$label = 'Activo'; $color = 'success';}
							elseif ($doc->ARC_ESTADO == 2) {$label = 'Inactivo';$color = 'danger';}

							if ($doc->ARC_PESO >= 1073741824) { 
								$bytes = number_format($doc->ARC_PESO / 1073741824, 2) . ' GB'; 
							} elseif ($doc->ARC_PESO >= 1048576) { 
								$bytes = number_format($doc->ARC_PESO / 1048576, 2) . ' MB'; 
							} elseif ($doc->ARC_PESO >= 1024) {
							 	$bytes = number_format($doc->ARC_PESO / 1024, 2) . ' KB'; 
							} elseif ($doc->ARC_PESO > 1) {
							 	$bytes = $doc->ARC_PESO . ' bytes'; 
							} elseif ($doc->ARC_PESO == 1) { 
								$bytes = $doc->ARC_PESO . ' byte'; 
							} else { 
								$bytes = '0 bytes'; 
							}
						?>
							<tr>
								...
								<td data-order="authentication">
									<div class="d-flex align-items-center">
										...
										<a href="<?php echo $doc->ARC_RUTA_DRIVE; ?>" target="_blank" class="text-gray-800 text-hover-primary"><?php echo $doc->ARC_NOMBRE; ?></a>
									</div>
								</td>
								<td><?php echo $bytes; ?></td>
								<td>
									<?php 
									if (!empty($doc->ARC_FEC_REGISTRO)) {
										echo date("d/m/Y h:m:s", strtotime($doc->ARC_FEC_REGISTRO));
									}
									?>									
								</td>
								<td><span class="badge <?php echo 'badge-'.$color; ?>"><?php echo $label; ?></span></td>									
								<td class="text-end" data-kt-filemanager-table="action_dropdown" style="display:none;">
									<div class="d-flex justify-content-end">
										<div class="ms-2">
											...
											<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-150px py-4" data-kt-menu="true">
												...
												<!--begin::Menu item-->
												<div class="menu-item px-3">
													<a href="#" class="menu-link text-danger px-3">Eliminar</a>
												</div>
												<!--end::Menu item-->
											</div>
											<!--end::Menu-->
...
$this->load->view('gestion/modulo_cliente/modal/mAddDoc');
?>
